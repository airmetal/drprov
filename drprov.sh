...
x=0
#echo "SERVER MAPPING FILE FOR CONSISTENCY GROUP $1 WITH PROD DATACENTER $2" > dr-server-mapping
echo "*******************************************************" >> dr-server-mapping
echo "Source_MCP,Source_VM_Name,Source_VM_IPV4,Source_VM_IPV6,Source_VM_OS,Source_VM_Displayname,Target_MCP,Target_VM_Name,Target_VM_IPV4,Target_VM_IPV6,Target_VM_OS,Target_VM_Displayname" > dr-server-mapping
...
while read p; do
  ...
  if [ $value == $2 ]
    then
        ...
        #echo "PROD server" >> dr-server-mapping
	...
	echo -n $value, >> dr-server-mapping
	...
	echo -n $dest | jq .[0].datacenterId | sed -e 's/^"//'  -e 's/"$//' |tr '\n' ',' >> dr-server-mapping
	 ...
	 echo -n $dest | jq .[0].Name | sed -e 's/^"//'  -e 's/"$//' >> dr-server-mapping
